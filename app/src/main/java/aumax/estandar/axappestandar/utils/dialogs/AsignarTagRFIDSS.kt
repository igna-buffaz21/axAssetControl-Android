...
class AsignarTagRFIDSS(
    context: Context,
    private val subSector: SubSector,
    private val onTagAssigned: (subSector: SubSector, tagCodes: List<String>) -> Unit // callback cuando se confirme
) : Dialog(context) {
    ...
    private fun setupViews() {
        ...
        binding.tvActivoNombre.text = subSector.name
        binding.tvActivoCodigo.text = "Código: ${subSector.tagRfid}"
        ...
        binding.btnConfirmar.setOnClickListener {
            if (tagList.isNotEmpty()) {
                onTagAssigned(subSector, tagList.toList())
                dismiss()
            }
        }
    ...
    companion object {
        fun show(
            context: Context,
            subSector: SubSector,
            onTagAssigned: (subSector: SubSector, tagCodes: List<String>) -> Unit
        ) : AsignarTagRFIDSS {
            val dialog = AsignarTagRFIDSS(context, subSector, onTagAssigned)
            dialog.show()
            return dialog
        }
    }
