...
class ControlOfflineActivity(
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        _oAxLector = AxLector(this@ControlOfflineActivity, Configuracion.MODO_LECTURA.RFID) //creamos el objeto que va a manejar todo el RFID
                    ...
                    if (leerTagSS && !leerTagsControl) {
                        ...
                        when {
                            !leerTagSS && !leerTagsControl -> {
                                Toast.makeText(this@ControlOfflineActivity, "Lectura no iniciada, Lectores desactivados", Toast.LENGTH_SHORT).show()
                            }
                            !leerTagSS -> {
                                Toast.makeText(this@ControlOfflineActivity, "Lectura no iniciada, Presione en Leer Tag Subsector", Toast.LENGTH_SHORT).show()
                            }
                            !leerTagsControl -> {
                                Toast.makeText(this@ControlOfflineActivity, "Lectura no iniciada, Presione en Iniciar", Toast.LENGTH_SHORT).show()
                            }
                        }
                    }
                }
                ...
                if (leerTagSS) {
                    ...
                    if (listTagsLeidos.size > 1) {
                        ...
                        binding.btnLeerTagSubsector.setBackgroundColor(
                            ContextCompat.getColor(this@ControlOfflineActivity, R.color.black)
                        )

                        binding.btnLeerTagSubsector.text = "Leer Tag Subsector"

                        runOnUiThread {
                            Toast.makeText(this@ControlOfflineActivity, "Se detectaron múltiples tags, intenta nuevamente.", Toast.LENGTH_LONG).show()
                        }
        ...
        lifecycleScope.launch {
            ...
            response
                ...
                .onFailure { error ->
                    ...
                    Toast.makeText(this@ControlOfflineActivity, "Error al cargar los activos", Toast.LENGTH_SHORT).show()
                }
        }
    }
        ...
        lifecycleScope.launch {
            ...
            reponse
                .onSuccess { id ->
                    ...
                    Toast.makeText(this@ControlOfflineActivity, "Control guardado con exito", Toast.LENGTH_SHORT).show()

                    val idControl = id

                    crearDetallesControlEnCantidad(idControl!!.toInt())
                }
                .onFailure {
                    Log.d("REGISTRO CONTROL CREADO CON EXITO", "NO se creo ${reponse} ")
                    Toast.makeText(this@ControlOfflineActivity, "Error al crear el control", Toast.LENGTH_SHORT).show()
                }
        }
    }
    ...
    private fun crearDetallesControlEnCantidad(idControl: Int) {
        lifecycleScope.launch {
            ...
            val response = registroControlRepository.crearDetalleControlBD(detallesList)
